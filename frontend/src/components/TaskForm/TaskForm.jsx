...
const TaskForm = ({ fetchTasks }) => {
  ...
  const COMMANDS = ['crear tarea', 'guardar tarea', 'añadir tarea'];
  const CLOSE_COMMANDS = ['cerrar micrófono', 'detener micrófono'];
  ...
  const processText = (prevText, newText) => {
    const combined = prevText ? `${prevText} ${newText}` : newText;
    const words = combined.split(/\s+/);
    const filtered = words.filter((w, i, arr) => i === 0 || w.toLowerCase() !== arr[i - 1].toLowerCase());
  ...
  useEffect(() => {
    ...
    if (!recognitionRef.current) {
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        // Comando de cerrar micrófono
        if (CLOSE_COMMANDS.some(cmd => lower.includes(cmd))) {
          console.log('Comando por voz detectado: Cerrar micrófono');
          stopListening();
          setError('');
          return;
        }

        // Comando de crear tarea
        const cmdCrear = COMMANDS.find(c => lower.includes(c));
        if (cmdCrear) {
          const before = transcript.slice(0, lower.indexOf(cmdCrear)).trim();
          if (before) {
            if (activeFieldRef.current === 'title') {
              setTitle(prev => processText(prev, before));
            } else {
              setDescription(prev => processText(prev, before));
            }
          }
          console.log('Comando por voz detectado:', cmdCrear);
          stopListening();
          setError('');
        ...
        // Si no era comando, añadimos al campo activo
        if (activeFieldRef.current === 'title') {
          setTitle(prev => processText(prev, transcript));
        } else {
          setDescription(prev => processText(prev, transcript));
        }
      };

      recognitionRef.current.onerror = (e) => {
        console.error('Error micrófono:', e.error);
        setError(`Error en el micrófono: ${e.error}`);
        stopListening();
      };
  ...
  const startListening = async (field) => {
    ...
    if (recognitionRef.current && (isListeningTitle || isListeningDesc)) {
      await new Promise(res => {
        recognitionRef.current.onend = res;
        recognitionRef.current.stop();
      });
    }
    setTimeout(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('/api/tasks', { title, description, completed: false, id_user });
      setTitle('');
      setDescription('');
      setError('');
      fetchTasks();
  ...
  return (
    ...
    >
      ...
      {(isListeningTitle || isListeningDesc) && (
        <div className="bg-gray-600/80 text-white p-3 rounded-lg text-sm flex items-center gap-2 animate-fade-in">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"/>
          </svg>
          <span>Antes de finalizar, diga “crear tarea” para añadirla o “cerrar micrófono” para detener.</span>
        </div>
      )}
        ...
        <div className="relative">
          ...
          <button
            ...
            className={`absolute right-3 top-1/2 transform -translate-y-1/2 p-2 rounded-full ${
              isListeningTitle ? 'bg-red-500 animate-pulse' : 'bg-white/30 hover:bg-white/40'
            } transition-all`}
            aria-label="Micrófono para título"
          >
        ...
        <div className="relative">
          ...
          <button
            ...
            className={`absolute right-3 bottom-4 p-2 rounded-full ${
              isListeningDesc ? 'bg-red-500 animate-pulse' : 'bg-white/30 hover:bg-white/40'
            } transition-all`}
            aria-label="Micrófono para descripción"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"/>
